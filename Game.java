...
public class Game {
	...
	public void play(){
		...
		System.out.println("How much are you betting? 1-5");
		...
		if(hand.size()==0){
			...
			System.out.println("Replace the fifth card? 1 is yes and 0 is no.");
			if(input.nextInt()==1){
				p.removeCard(hand.get(4));
				p.addCard(cards.deal());
			}
			System.out.println("Replace the fourth card? 1 is yes and 0 is no.");
			if(input.nextInt()==1){
				p.removeCard(hand.get(3));
				p.addCard(cards.deal());
			}
			System.out.println("Replace the third card? 1 for yes and 0 for no.");
			if(input.nextInt()==1){
				p.removeCard(hand.get(2));
				p.addCard(cards.deal());
			}
			System.out.println("Replace the second card? 1 for yes and 0 for no.");
			if(input.nextInt()==1){
				p.removeCard(hand.get(1));
				p.addCard(cards.deal());
			}
			System.out.println("Replace the first card? 1 for yes and 0 for no.");
			if(input.nextInt()==1){
		...
		System.out.println("Finalized hand" +hand);
		System.out.println(checkHand(hand));
		System.out.println("Your bankroll after this round is" +p.getBankroll());
		System.out.println("Play Again? 1 is yes and 0 is no.");
		if(input.nextInt()==1){
			p.clearHand();
			play();
		}
	...
	public String checkHand(ArrayList<Card> hand){
		...
		String report = "";
		if(royalFlush(hand)){
			report = "Congrats You got a Royal FLUSH!!!!!!!";
			p.winnings(250.0);
			return report;
		}
			...
			report="You got a Four of a Kind!";
			p.winnings(25.0);
			return report;
		}
		else if (fullhouse(hand)){
			report = "You get Full house!!!!";
			p.winnings(6.0);
			return report;
		}
		else if (flush(hand)){
			...
			report="You get Three of a KIND!!!!!";
			p.winnings(3.0);
			return report;
		}
		else if (twoPairs(hand)){
			report="You get two Pairs";
			p.winnings(2.0);
			return report;
		}
